  ...
  head: () => ({
    meta: [
      ...
      },
      {
        name: "description",
        content:
          "Add, edit and track institute students with course fees, paid and remaining amounts.",
      },
      {
        property: "og:title",
        content: "Students — Institute Fee Management",
      },
      {
        property: "og:description",
        content:
          "Manage student records, course fees and payment due dates.",
      },
    ],
  }),
...
function StudentsPage() {
  const { students, studentsError, refresh } = useAppData();
  ...
  const grouped = useMemo(() => {
    ...
    filtered.forEach((student) => {
      ...
      if (!batches.has(student.batch)) {
        batches.set(student.batch, []);
      }
    ...
    >
      <Card className="mb-4">
        <div className="grid gap-3 lg:grid-cols-[minmax(240px,1fr)_220px_180px_auto] lg:items-end">
          <div className="relative w-full">
            ...
            <Input
              className="pl-9"
              placeholder="Search name, ID, course, batch..."
              value={q}
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          ...
          {students.length} students · grouped by
          course and batch
        </p>
      </Card>
                      ...
                      >
                        ...
                        {batchStudents.map((s) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-[oklch(0.45_0.13_155)]">
                              {formatINR(
                                s.paidFee,
                              )}
                            </td>
                              ...
                              <div className="flex justify-end gap-1">
                                ...
                                <button
                                  ...
                                  onClick={async () => {
                                    if (
                                      confirm(
                                        `Delete ${s.name}? This also removes their payments.`,
                                      )
                                    ) {
                                      try {
      ...
      {studentsError && (
        <p className="mt-3 text-sm text-destructive">
          Unable to load students from the database: {studentsError}
        </p>
      )}
          ...
          <dl className="grid gap-3 sm:grid-cols-2">
            {[
              ...
              [
                "Admission Date",
                formatDate(
                  viewing.admissionDate,
                ),
              ],
              [
                "Instalment Date",
                formatDate(
                  viewing.instalmentDate,
                ),
              ],
              [
                "Total Course Fee",
                formatINR(viewing.totalFee),
              ],
              [
                "Total Paid",
                formatINR(viewing.paidFee),
              ],
              [
                "Remaining Fee",
                formatINR(
...
function StudentForm({
  ...
  const courseOptions =
    ...
    !COURSE_OPTIONS.includes(
      f.course as (typeof COURSE_OPTIONS)[number],
    )
  ...
  const batchOptions =
    f.batch &&
    !BATCH_OPTIONS.includes(f.batch)
    ...
    >
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Instalment Date">
            <Input
              type="date"
              value={f.instalmentDate}
              onChange={(e) =>
                set(
                  "instalmentDate",
                  e.target.value,
          ...
          <Field label="Remaining Fee (auto)">
            <Input
              disabled
