...
export const Route = createFileRoute("/payment-history")({
  head: () => ({
    meta: [
      {
        title: "Payment History — Institute Fee Management",
      },
    ],
  }),
...
function PaymentHistory() {
  ...
  const rows = useMemo(() => {
    ...
    return payments
      .map((p) => ({
        p,
        s: students.find((x) => x.id === p.studentId),
      }))
      .filter(({ s }) => !!s)
      .filter(({ s, p }) =>
        !t
  ...
  return (
    <AppLayout
      ...
      subtitle={`${rows.length} payments · ${formatINR(total)} collected`}
    >
      <Card className="mb-4">
          ...
          <tbody>
            ...
            {rows.map(({ p, s }) => (
              ...
              >
                ...
                <td className="px-4 py-3 font-medium text-[oklch(0.45_0.13_155)]">
                  {formatINR(p.amount)}
                </td>
      ...
      {paymentsError && (
        <p className="mt-3 text-sm text-destructive">
          Unable to load payments from the database: {paymentsError}
        </p>
      )}
    </AppLayout>
  );
    ...
    >
      <form
        ...
        onSubmit={async (e) => {
          ...
          if (amount > allowed) {
            alert(
              "Payment cannot be more than the remaining fee.",
            );
            return;
          }
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          {f.mode === "UPI" && (
            <Field label="UPI ID / Reference ID">
              <Input
